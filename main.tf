...
resource "helm_release" "metallb" {
  ...
  provisioner "local-exec" {
    command = "kubectl apply -f ./helm_values/values-metallb.yaml"
  }
}
...
resource "helm_release" "ingress-nginx" {
  ...
  values = [
    file("./helm_values/values-nginx.yaml")
  ]
...
resource "helm_release" "prometheus-stack" {
  ...
  values = [
    file("./helm_values/values-prometheus.yaml")
  ]
...
/*
resource "null_resource" "echo-deployment" {
  provisioner "local-exec" {
    command = "kubectl apply -f ./echo-deployment.yaml"
  }

  depends_on = [ helm_release.ingress-nginx, helm_release.metallb ]
}
*/